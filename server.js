...
{
name: '003_cleanup_and_create_witness1',
sql: `
-- Remove all existing users except witness1 admin
DELETE FROM users WHERE handle_number != 'witness1';

-- Create witness1 admin account if it doesn't exist
...
-- Reset user number sequence
SELECT setval('user_number_seq', 1, true);
`
}
];
...
// Vote on content - Enhanced to return user's current vote
...
// Get user's votes for content (new endpoint)
app.get('/api/votes/:targetType/:targetId', authRequired, async (req, res) => {
try {
...
const handle_number = `${handle.toLowerCase()}${nextval}`;
...
`, [handle.toLowerCase(), nextval, handle_number, password_hash, email || null, creed || null, 'member', 'active']);
...
'SELECT id, handle_number, password_hash, field_cred, is_admin, active, creed FROM users WHERE handle_number = $1',
[handle_number]
);
...
'SELECT id, handle_number, password_hash, field_cred, is_admin, active, creed FROM users WHERE handle = $1 ORDER BY number ASC',
[handlePart]
);
...
// Get threads for a board - Enhanced with vote data
...
LEFT JOIN votes v ON v.target_type = 'thread' AND v.target_id = t.id AND v.voter_id = $2
...
// Get thread with posts - Enhanced with vote data
...
LEFT JOIN votes v ON v.target_type = 'post' AND v.target_id = p.id AND v.voter_id = $2
...
// Get chat messages for a room - Enhanced with vote data
...
LEFT JOIN votes v ON v.target_type = 'message' AND v.target_id = m.id AND v.voter_id = $2
WHERE r.key = $1 AND u.active != 'deleted'
ORDER BY m.created_at DESC
LIMIT $3
`, [key, req.user.id, limit]);
...
(SELECT COUNT(*) FROM private_messages WHERE chat_id = pc.id) as message_count,
COALESCE(
...
app.get('/api/admin/private-chats/:chatId/messages', authRequired, adminRequired, async (req, res) => {
try {
const chatId = parseInt(req.params.chatId);
...
// Verify chat exists
const { rows: [chat] } = await query('SELECT id FROM private_chats WHERE id = $1', [chatId]);
if (!chat) {
...
INSERT INTO private_messages (chat_id, sender_id, body)
VALUES ($1, $2, $3)
`, [chatId, req.user.id, `Admin ${req.user.handle_number} has joined this conversation for monitoring purposes.`]);
