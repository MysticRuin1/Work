...
`, [cleanHandle, nextval, handle_number, password_hash, email || null, creed || null, 'member',
'active']);
...
const { rows: [user] } = await query('SELECT password_hash FROM users WHERE id = $1',
[req.user.id]);
...
const userId = parseInt(req.params.userId);
const { reason } = req.body;
const { rows: [user] } = await query('SELECT handle_number FROM users WHERE id = $1',
[userId]);
if (!user) {
return res.status(404).json({ error: 'User not found' });
}
