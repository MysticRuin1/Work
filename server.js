...
sql: `
DELETE FROM users WHERE handle_number = 'witness1' OR handle = 'witness';
INSERT INTO users (handle, number, handle_number, password_hash, creed, member, active, is_admin, is_moderator, field_cred)
VALUES ('witness', 1, 'witness1', '${await bcrypt.hash('admin123', 12)}', 'judgment-day', 'admin', 'active', true, true, 10000);
`
},
{
...
((SELECT id FROM chat_rooms WHERE key = 'global'), 1, 'All hunters report status.', '2024-01-15T20:00:00Z'),
((SELECT id FROM chat_rooms WHERE key = 'global'), 1, 'Good work. Maintain surveillance protocols. All units stay vigilant.', '2024-01-15T20:12:00Z');

-- Insert chat history for firelight
INSERT INTO messages (room_id, author_id, body, created_at) VALUES
((SELECT id FROM chat_rooms WHERE key = 'firelight'), 1, 'Firelight cell coordination - new purification protocols have been distributed.', '2024-01-15T19:00:00Z');

-- Insert chat history for judgment-day
INSERT INTO messages (room_id, author_id, body, created_at) VALUES
((SELECT id FROM chat_rooms WHERE key = 'judgment-day'), 1, 'Judgment Day operations are proceeding on schedule. All prophecies align.', '2024-01-15T18:45:00Z');

-- Insert chat history for triage
INSERT INTO messages (room_id, author_id, body, created_at) VALUES
((SELECT id FROM chat_rooms WHERE key = 'triage'), 1, 'Triage unit standing by. Medical supplies restocked and ready for deployment.', '2024-01-15T17:30:00Z');

-- Insert chat history for unity
INSERT INTO messages (room_id, author_id, body, created_at) VALUES
((SELECT id FROM chat_rooms WHERE key = 'unity'), 1, 'Unity negotiations with local authorities proceeding smoothly.', '2024-01-15T16:45:00Z');

-- Insert chat history for vigil
INSERT INTO messages (room_id, author_id, body, created_at) VALUES
((SELECT id FROM chat_rooms WHERE key = 'vigil'), 1, 'Vigil patrol routes optimized. Night watch schedules distributed.', '2024-01-15T19:45:00Z');

-- Insert chat history for vitalis
INSERT INTO messages (room_id, author_id, body, created_at) VALUES
((SELECT id FROM chat_rooms WHERE key = 'vitalis'), 1, 'Vitalis research into supernatural physiology yielding promising results.', '2024-01-15T15:30:00Z');

-- Insert admin chat history
INSERT INTO messages (room_id, author_id, body, created_at) VALUES
((SELECT id FROM chat_rooms WHERE key = 'admin'), 1, 'System maintenance completed. All security protocols are operational.', '2024-01-15T14:00:00Z');
`
}
];
