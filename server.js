...
sql: `
ALTER TABLE users ADD COLUMN IF NOT EXISTS is_moderator BOOLEAN
DEFAULT false;
`
},
{
...
member, active, is_admin, is_moderator, field_cred)
...
member, active, is_admin, is_moderator, field_cred)
VALUES
('hunter', 1, 'hunter001', '${await bcrypt.hash('test123', 12)}', 'vigil', 'member',
'active', false, false, 250),
('tracker', 7, 'tracker007', '${await bcrypt.hash('password123', 12)}', 'firelight',
'moderator', 'active', false, true, 1500)
...
await query(`SELECT setval('user_number_seq', COALESCE((SELECT MAX(number)
FROM users), 1), true)`);
...
`, [handle, nextval, handle_number, password_hash, email || null, creed || null, 'member',
'active']);
// Sign JWT
...
'SELECT id, handle_number, password_hash, field_cred, creed, is_admin,
is_moderator, active, created_at FROM users WHERE handle_number = $1',
[handle_number]
);
let user = null;
...
'SELECT id, handle_number, password_hash, field_cred, creed, is_admin,
is_moderator, active, created_at FROM users WHERE handle = $1 ORDER BY number ASC',
[handlePart]
);
...
FROM users WHERE id = $1 AND active != \'deleted\'',
[req.user.id]
);
if (!user) {
...
(SELECT COUNT(*) FROM threads WHERE author_id = users.id) as
active_threads,
(SELECT COUNT(*) FROM posts WHERE author_id = users.id) as post_count,
(SELECT COUNT(*) FROM messages WHERE author_id = users.id) as
message_count,
true as is_online
FROM users
WHERE active = 'active'
...
COUNT(CASE WHEN recipient_id = $1 AND read_at IS NULL THEN 1 END) as
unread_count,
MIN(id) as id
...
console.log(`Admin Moderator Update: ${user.handle_number} moderator =
${is_moderator}`);
res.json({
success: true,
user: {
