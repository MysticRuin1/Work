...
console.log(`Field Cred: ${user.handle_number} ${credChange > 0 ? '+' : ''}${credChange}
(${voteType}vote on ${targetType}) -> ${user.field_cred}`);
}
} catch (error) {
...
{
name: '002_init_chat_rooms_and_boards',
sql: `
...
('Firelight', 'firelight', 'Purification and sanctification operations'),
('Judgment Day', 'judgment-day', 'Prophetic visions and revelations'),
('Triage', 'triage', 'Medical support and field treatment'),

('Unity', 'unity', 'Public relations and civilian coordination'),
('Vigil', 'vigil', 'Surveillance and patrol operations'),
('Vitalis', 'vitalis', 'Research and biological studies'),
('Admin Board', 'admin', 'Administrative discussions and system updates')
ON CONFLICT (key) DO NOTHING;
`
},
{
name: '003_create_witness1_admin',
sql: async () => {
// Delete any existing witness user first
await query(`DELETE FROM users WHERE handle_number = 'witness1' OR handle = 'witness'`);

// Create the admin user and get the actual ID
const passwordHash = await bcrypt.hash('admin123', 12);
const { rows: [adminUser] } = await query(`
INSERT INTO users (handle, number, handle_number, password_hash, creed, member, active,
is_admin, is_moderator, field_cred)
VALUES ('witness', 1, 'witness1', $1, 'judgment-day', 'admin',
'active', true, true, 10000)
RETURNING id
`, [passwordHash]);
return adminUser.id; // Return the actual admin user ID
}
},
{
name: '004_seed_chat_history',
sql: (adminUserId) => `
-- Insert chat history for global
INSERT INTO messages (room_id, author_id, body, created_at) VALUES
((SELECT id FROM chat_rooms WHERE key = 'global'), ${adminUserId}, 'All hunters report status.',
'2024-01-15T20:00:00Z'),
((SELECT id FROM chat_rooms WHERE key = 'global'), ${adminUserId}, 'Good work. Maintain surveillance
protocols. All units stay vigilant.', '2024-01-15T20:12:00Z');
-- Insert chat history for firelight
INSERT INTO messages (room_id, author_id, body, created_at) VALUES
((SELECT id FROM chat_rooms WHERE key = 'firelight'), ${adminUserId}, 'Firelight cell coordination - new
purification protocols have been distributed.', '2024-01-15T19:00:00Z');
-- Insert chat history for judgment-day
INSERT INTO messages (room_id, author_id, body, created_at) VALUES
((SELECT id FROM chat_rooms WHERE key = 'judgment-day'), ${adminUserId}, 'Judgment Day operations
are proceeding on schedule. All prophecies align.', '2024-01-15T18:45:00Z');
-- Insert chat history for triage
INSERT INTO messages (room_id, author_id, body, created_at) VALUES
((SELECT id FROM chat_rooms WHERE key = 'triage'), ${adminUserId}, 'Triage unit standing by. Medical
supplies restocked and ready for deployment.', '2024-01-15T17:30:00Z');
-- Insert chat history for unity
INSERT INTO messages (room_id, author_id, body, created_at) VALUES

((SELECT id FROM chat_rooms WHERE key = 'unity'), ${adminUserId}, 'Unity negotiations with local
authorities proceeding smoothly.', '2024-01-15T16:45:00Z');
-- Insert chat history for vigil
INSERT INTO messages (room_id, author_id, body, created_at) VALUES
((SELECT id FROM chat_rooms WHERE key = 'vigil'), ${adminUserId}, 'Vigil patrol routes optimized. Night
watch schedules distributed.', '2024-01-15T19:45:00Z');
-- Insert chat history for vitalis
INSERT INTO messages (room_id, author_id, body, created_at) VALUES
((SELECT id FROM chat_rooms WHERE key = 'vitalis'), ${adminUserId}, 'Vitalis research into supernatural
physiology yielding promising results.', '2024-01-15T15:30:00Z');
-- Insert admin chat history
INSERT INTO messages (room_id, author_id, body, created_at) VALUES
((SELECT id FROM chat_rooms WHERE key = 'admin'), ${adminUserId}, 'System maintenance completed. All
security protocols are operational.', '2024-01-15T14:00:00Z');
`
}
];
...
if (step.name === '003_create_witness1_admin') {
// Special handling for the admin creation step
adminUserId = await step.sql();
} else if (step.name === '004_seed_chat_history' && adminUserId) {
// Use the actual admin user ID for seeding
await query(step.sql(adminUserId));
} else if (typeof step.sql === 'string') {
...
await query(`SELECT setval('user_number_seq', COALESCE((SELECT MAX(number) FROM
users), 1), true)`);
console.log('Migrations completed successfully');
console.log('Admin Account: witness1 / admin123');
} catch (error) {
...
const updateQuery = `UPDATE ${tableName} SET upvotes = $1, downvotes = $2 WHERE id =
$3`;
...
// Register - matches HTML form fields
...
// Generate unique handle number - using 3 digits to match HTML
const { rows: [{ nextval }] } = await query('SELECT nextval(\'user_number_seq\')');
const paddedNumber = nextval.toString().padStart(3, '0');
const handle_number = `${handle}${paddedNumber}`;
// Hash password
const password_hash = await bcrypt.hash(password, 12);
// Create user
const { rows: [user] } = await query(`
INSERT INTO users (handle, number, handle_number, password_hash, email, creed, member,
active)
VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
RETURNING id, handle_number, field_cred, creed, is_admin, is_moderator, created_at
`, [handle, parseInt(nextval), handle_number, password_hash, email || null, creed || null,
'member', 'active']);
// Sign JWT
...
// Login - matches HTML form fields (handle_number, password)
...
'SELECT id, handle_number, password_hash, field_cred, creed, is_admin, is_moderator, active,
created_at FROM users WHERE handle_number = $1',
[handle_number]
);
let user = null;
...
'SELECT id, handle_number, password_hash, field_cred, creed, is_admin, is_moderator, active,
created_at FROM users WHERE handle = $1 ORDER BY number ASC',
[handlePart]
);
...
try {
const { rows: [user] } = await query(
'SELECT id, handle_number, field_cred, creed, is_admin, is_moderator, created_at FROM
users WHERE id = $1 AND active != \'deleted\'',
[req.user.id]
);
if (!user) {
...
`, [original.subject]);
console.log(`Admin deleted private conversation: ${original.subject} by
...
const { rows: roomRows } = await query(`SELECT id FROM chat_rooms WHERE key=$1`,
[roomKey]);
...
console.log(`Chat message: [${roomKey}] ${socket.user.handle_number}: ${body.substring(0,
50)}...`);
...
console.log('═══════════════════════════════════════════');
console.log('Admin Account: witness1 / admin123');
console.log('Chat history has been seeded in database');
console.log('Images will be saved and viewable in chat');
console.log('═══════════════════════════════════════════');
