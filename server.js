    ...
    const steps = [
      ...
      },
      {
        ...
        sql: `
          ...
          DO $$
          BEGIN
            -- Add name column to boards if missing
            IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'boards' AND column_name = 'name') THEN
              ALTER TABLE boards ADD COLUMN name TEXT;
            END IF;
            
            -- Add image_path column to messages if missing
            IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'messages' AND column_name = 'image_path') THEN
              ALTER TABLE messages ADD COLUMN image_path TEXT;
            END IF;
            
            -- Add upvotes column to messages if missing
            IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'messages' AND column_name = 'upvotes') THEN
              ALTER TABLE messages ADD COLUMN upvotes INT DEFAULT 0;
            END IF;
            
            -- Add downvotes column to messages if missing
            IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'messages' AND column_name = 'downvotes') THEN
              ALTER TABLE messages ADD COLUMN downvotes INT DEFAULT 0;
            END IF;
            
            -- Add upvotes column to threads if missing
            IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'threads' AND column_name = 'upvotes') THEN
              ALTER TABLE threads ADD COLUMN upvotes INT DEFAULT 0;
            END IF;
            
            -- Add downvotes column to threads if missing
            IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'threads' AND column_name = 'downvotes') THEN
              ALTER TABLE threads ADD COLUMN downvotes INT DEFAULT 0;
            END IF;
            
            -- Add signal_type column to threads if missing
            IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'threads' AND column_name = 'signal_type') THEN
              ALTER TABLE threads ADD COLUMN signal_type TEXT;
            END IF;
            
            -- Add upvotes column to posts if missing
            ...
            -- Add downvotes column to posts if missing
