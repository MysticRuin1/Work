...
{
name: '003_add_missing_columns',
sql: `
-- Add pinned column if it doesn't exist
DO $$
BEGIN
IF NOT EXISTS (SELECT 1 FROM information_schema.columns
WHERE table_name = 'threads' AND column_name = 'pinned') THEN
ALTER TABLE threads ADD COLUMN pinned BOOLEAN DEFAULT false;
END IF;
END $$;
`
},
{
name: '004_add_user_deletion_columns',
sql: `
-- Add deleted_at and deletion_reason columns if they don't exist
DO $$
BEGIN
IF NOT EXISTS (SELECT 1 FROM information_schema.columns
...
console.log(`Fixing witness1 password with hash: ${password_hash.substring(0,
20)}...`);
return `
...
VALUES ('witness', 1, 'witness1', '${password_hash}', 'vigil', 'admin', 'active', true,
999)
...
console.log(`Registering user: ${handle_number} with hash: ${password_hash.substring(0,
20)}...`);
...
`, [cleanHandle, nextval, handle_number, password_hash, email || null, creed || null,
'member', 'active']);
...
// Login - Fixed with better error handling and consistent password comparison
...
'SELECT id, handle_number, password_hash, field_cred, is_admin, active, creed FROM
users WHERE LOWER(handle_number) = $1',
[cleanHandleNumber]
);
...
// Check password - ensure we're comparing exactly what was provided
...
// Get thread with posts - FIXED: Safe pinned column handling
app.get('/api/threads/:id', async (req, res) => {
try {
const { id } = req.params;

// Get thread with conditional pinned selection
const { rows: [thread] } = await query(`
...
WHERE id = $${paramCount}
RETURNING id, sticky, locked, COALESCE(pinned, false) as pinned
`, values);
...
// Verify user has access to this chat OR is an admin
const { rows: [chat] } = await query(`
SELECT pc.id FROM private_chats pc
JOIN users u1 ON pc.user1_id = u1.id
JOIN users u2 ON pc.user2_id = u2.id
WHERE pc.id = $1 AND (u1.handle_number = $2 OR u2.handle_number = $2 OR $3 =
true)
`, [chatId, req.user.handle_number, req.user.is_admin]);
...
active, created_at,
CASE WHEN deleted_at IS NOT NULL THEN deleted_at ELSE NULL END as deleted_at,
CASE WHEN deletion_reason IS NOT NULL THEN deletion_reason ELSE NULL END as deletion_reason,
